...
%% === 'control' parameter ===
a = 110;   a1 = 90;
tau = 0.2;  % tunning parameter for Copula function
...
%% ------------* Case B *------------
% Beta = 1.0*[1.0;0;-1.0;0;0;1.0;zeros(p-6,1)];  
% PHI = 1.0*[0;1.0;0;-1.0;zeros(p-4,1)];   
...
for iter = 1:N
    ...
    [T,Z,Delta,T1,Z1,Delta1] = survival_data(n,Beta,PHI,mu,sigma,tau,iter); % simulation data
    Censorrate1(iter) = 1-mean(Delta);

    [ini_beta(:,iter),ini_phi(:,iter)] =...
        initial_beta(n,Z,1e-5,Delta,a1);
end

% mean(Censorrate1)   % censoring rate
beta0 = mean(ini_beta,2);  % initial value for beta
phi0 = mean(ini_phi,2);    % initial value for phi
...
for iter = 1:N
    ...
    [ID,binedg,binwv] = discrBinNA(T1,n0);
    [psix, Psi] = Psi_psi(binwv,ID);
    RT = mat1(psix,T1,1e-5);
    ...
    %     MPL(n,ini_beta(:,iter),ini_phi(:,iter),Z1,1e-5,psix,Psi,RT,Delta1,a,tau);
    [mpl(:,iter),phi(:,iter),seta(:,iter),gama(:,iter),H_T(:,iter),H_C(:,iter)] =...
        MPL(n,beta0,phi0,Z1,1e-5,psix,Psi,RT,Delta1,a,tau);

    %% ------------* MSE *------------
    se_pl(iter) = (pl(:,iter)-Beta)'*sigma*(pl(:,iter)-Beta); 
    se_mpl(iter) = (mpl(:,iter)-Beta)'*sigma*(mpl(:,iter)-Beta); 
    se_phi(iter) = (phi(:,iter)-PHI)'*sigma*(phi(:,iter)-PHI); 
    se_oracle(iter)= (oracle_beta(:,iter)-Beta(index))'*sigma(index,index)*(oracle_beta(:,iter)-Beta(index));   
    se_full(iter)= (full_beta(:,iter)-Beta)'*sigma*(full_beta(:,iter)-Beta); 

    %% ------------* Covariance matrix *------------
    [cov_beta(:,iter),cov_phi(:,iter)] = cov_matrix(...
        mpl(:,iter),phi(:,iter),seta(:,iter)',gama(:,iter)',Z1,Delta1,n,Psi,index,index1,tau);
...
PL_SD_beta = s1';
%PL_ASD_beta = mean(sqrt(PL_cov_beta),2);
b = sqrt(PL_cov_beta);
PL_ASD_beta = [median(b(1,:),'all')./0.6745 median(b(2,:),'all')./0.6745 median(b(3,:),'all')./0.6745]';
...
Result_MPL_beta = [mean(mpl(index,:),2) mean(mpl(index,:),2)-Beta(index) MPL_SD_beta MPL_ASD_beta CP_MPL_beta]
Result_MPL_phi = [mean(phi(index1,:),2) mean(phi(index1,:)-PHI(index1),2) MPL_SD_phi MPL_ASD_phi CP_MPL_phi]
Result_PL_beta = [mean(pl(index,:),2) mean(pl(index,:),2)-Beta(index) PL_SD_beta PL_ASD_beta CP_PL_beta]
...
p = length(Beta);
ZZ = mvnrnd(mu,sigma,n);  % % n*p covariates matrix
alph = copulaparam('Frank',tau);  % %  the scalar parameter alpha.
u = copularnd('Frank',alph,n);
% lamt = 2.3; % tau = 0.5;
lamt = 2.0;   %tau = 0.2;
% lamt = 2.1;   %tau = 0.2;
Death_time = sqrt(-lamt^2*log(1-u(:,1))./exp(ZZ*Beta));  % death time or failure time
C = -5*log(1-u(:,2))./exp(ZZ*Phi);

statu = (Death_time <= C);
TT = min(Death_time,C);     % survial time

[T,I] = sort(TT,'descend');  % sorting the time-'descend'
% Y = bsxfun(@ge,T,T');      % at risk process
Z = ZZ(I,:);
status = statu(I);

[T1,I1] = sort(TT,'ascend');  % sorting the time-'ascend'
...
%-- binedg
function  [binID,binedg,binwv] = discrBinNA(T, binCount)
...
%                 Psi_psi()
% ============================================================
%% #piecewise constant basis/cumulative function for the non-parametric baseline hazard
function [psi, Psix] = Psi_psi(binwv,ID)
n = length( ID );
m = max( ID );
psi = zeros(n,m);
Psix =zeros(n,m);
...
%                 mat1()
% ============================================================
%% First order difference%---%For piecewise constant
...
function [opt_beta,opt_phi,opt_seta,opt_gama,H_T,H_C] = MPL(n,ini_beta,ini_phi,Z,r,psix,Psi,RT,Delta,a,tau)
[~,p] = size(Z);
% tau: Kendall's tau
alpha = copulaparam('Frank',tau);  % %  the scalar parameter alpha.
...
k=1; err1=0; err2=0; err3=0; err4=0; %tk1= 16; tk2= 24;
% tk1= 160; tk2= 160;  % p = 60
% tk1= 24; tk2= 16;  % p = 100
...
while k<=1000 && (err1==0 ||err2==0 || err3==0 || err4==0)
    ...
    K1=( exp(alpha*(u+v))-exp(alpha*u) )./( exp(alpha*(u+v))-exp(alpha*u)-exp(alpha*v)+exp(alpha) ); % du
    K2=( exp(alpha*(u+v))-exp(alpha*v) )./( exp(alpha*(u+v))-exp(alpha*u)-exp(alpha*v)+exp(alpha) ); %dv
    K11=( alpha*( exp(alpha*(u+v))-exp(alpha*u)).*(exp(alpha)-exp(alpha*v)) )./( (exp(alpha*(u+v))-exp(alpha*u)-exp(alpha*v)+exp(alpha)).^2 ); %dudu
    K12=( alpha*exp(alpha*(u+v))*(exp(alpha)-1) )./( (exp(alpha*(u+v))-exp(alpha*u)-exp(alpha*v)+exp(alpha)).^2 ); %dudv=dvdu; %K21=K12
    K21 = K12;
    K22=( alpha*( exp(alpha*(u+v))-exp(alpha*v)).*(exp(alpha)-exp(alpha*u)) )./( (exp(alpha*(u+v))-exp(alpha*u)-exp(alpha*v)+exp(alpha)).^2 ); %dvdv
    ...
    uu = 0.5; % taking the value between 0 and 1
    gamm = 0.35; % taking the value between 0 and 0.5
    sigm = 0.35; % taking the value between 0 and 1
    ...
    while (t>0)
        ...
        LAM11 = Delta.*( max(K11,0)./K1 )+(1-Delta).*( K21./K2 );
        LAM22 = Delta.*( max(K22,0)./K2 )+(1-Delta).*( K12./K1 );
        

        DL_seta = sum( (Delta.*psix)./(sum(seta.*psix,2)+1e-5)- ...
            (exp(Z*beta).*(Delta+LAM1.*S_T)).*Psi,1)/n - lambda1*seta*RT;   % first partial derivative - seta

        DL_gama = sum( ((1-Delta).*psix)./(sum(seta.*psix,2)+1e-5)- ...
            (exp(Z*phi).*(1-Delta+LAM2.*S_C)).*Psi,1)/n - lambda2*gama*RT;  % first partial derivative - gama

        Ps1 = sum(( Delta.*exp(Z*beta) ).*Psi + ...
            ((LAM11.*S_T).*exp(Z*beta)).*Psi,1)+lambda1*max(seta*RT,0)+1e-7;
        Ps2 = sum(((1-Delta).*exp(Z*beta)).*Psi + ...
            ((LAM22.*S_C).*exp(Z*phi)).*Psi,1)+lambda2*max(gama*RT,0)+1e-7;
        ...
        seta1(seta1<=1e-7) = 1e-7;  
        gama1(gama1<=1e-7) = 1e-7;

...
for iter = 1:N
    ...
    [ini_beta2(:,iter),ini_phi2(:,iter)] =...
        initial_beta1(n,Z,1e-5/2,Delta,a);
...
for iter = 1:N
    ...
    [mpl2(:,iter),phi2(:,iter),seta2(:,iter),gama2(:,iter),H_T2(:,iter),H_C2(:,iter)]=...
        MPL1(n,n0,mean(ini_beta2,2),mean(ini_phi2,2),Z1,T1,1e-5,Delta1,a);
    
    se_mpl(iter) = (mpl(:,iter)-Beta)'*sigma*(mpl(:,iter)-Beta); % % mse
    se_mpl2(iter) = (mpl2(:,iter)-Beta)'*sigma*(mpl2(:,iter)-Beta); % % mse
   
    
    [cov_beta(:,iter),cov_phi(:,iter)] = cov_matrix(mpl(:,iter),phi(:,iter),seta(:,iter)',gama(:,iter)', ...
        T1,Z1,Delta1,n,n0,index,index1);

    [cov_beta2(:,iter),cov_phi2(:,iter)] = cov_matrix(mpl2(:,iter),phi2(:,iter),seta2(:,iter)',gama2(:,iter)', ...
        T1,Z1,Delta1,n,n0,index,index1);
...
Pcorr_mpl2 = sum((all(mpl2(index,:))).*(1-any(mpl2(setdiff(1:1:p, index),:))))/N;
MSE_mpl2 = mean(se_mpl2);
F_plus_mpl2 = sum(sum(mpl2(setdiff(1:1:p, index),:)~=0))/N;
F_minus_mpl2 = sum(sum(mpl2(index,:)==0))/N;
Size_mpl2 = sum(sum(mpl2(:,:)~=0))/N;

%% === Output ===
mean(Censorrate)   % Censoring rate

Criteria = [Pcorr_mpl  MSE_mpl  F_plus_mpl  F_minus_mpl  Size_mpl;
            Pcorr_mpl2 MSE_mpl2 F_plus_mpl2 F_minus_mpl2 Size_mpl]


[mean(mpl(index,:),2),mean(mpl2(index,:),2)]
BISE = [mean(mpl(index,:),2)-Beta(index) mean(mpl2(index,:),2)-Beta(index)]

s1=[std(mpl(1,:)),std(mpl(3,:)),std(mpl(6,:))];
s2=[std(mpl2(1,:)),std(mpl2(3,:)),std(mpl2(6,:))];

MPL_SD_beta = s1';
MPL_ASD_beta = mean(sqrt(cov_beta),2)

MPL2_SD_beta = s2';
MPL2_ASD_beta = mean(sqrt(cov_beta2),2)
...
CP_MPL2_beta = mean( and( mpl2(index,:)-1.96*sqrt(cov_beta2) <= repmat(Beta(index),1,N),...
    mpl2(index,:)+1.96*sqrt(cov_beta2) >= repmat(Beta(index),1,N) ),2);

Result_MPL_beta = [mean(mpl(index,:),2)-Beta(index) MPL_SD_beta MPL_ASD_beta CP_MPL_beta]

Result_MPL2_beta = [mean(mpl2(index,:),2)-Beta(index) MPL2_SD_beta MPL2_ASD_beta CP_MPL2_beta]
...
%                 initial_beta1()
% ============================================================
function [initial_beta,initial_phi]= initial_beta1(n,Z,r,status,a)
[~,p] = size(Z);
beta = zeros(p,1);
phi = zeros(p,1);
opt_BIC = 1e+10;
...
function [opt_beta,opt_phi,opt_seta,opt_gama,H_T,H_C] = MPL1(n,n0,ini_beta,ini_phi,Z,T,r,Delta,a)
[~,p] = size(Z);
m = n/n0;
tau = 0.5;  % % Kendall's tau
alph = copulaparam('Frank',tau);  % %  the scalar parameter alpha.
